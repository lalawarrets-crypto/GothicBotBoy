...
class ProfileCheckerCog(commands.Cog):
    ...
    @app_commands.command(name="testsearch", description="[Admin] Debug: buscar imagen por URL")
    @app_commands.describe(url="URL de imagen a buscar")
    async def testsearch(self, interaction: discord.Interaction, url: str):
        if not interaction.user.guild_permissions.administrator:
            return await interaction.response.send_message("❌ Solo admin.", ephemeral=True)
        await interaction.response.defer(ephemeral=True)
